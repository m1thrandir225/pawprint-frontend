...
const { data, isLoading, isError, error, refetch } = useQuery<AdoptionStatus[]>({
...
const { mutateAsync } = useMutation({
  mutationKey: ['createAdoptionStatus'],
  mutationFn: adoptionStatusService.createAdoptionStatus,
  onSuccess: () => {
    toast.success('Adoption status created successfully')
    refetch()
  },
  onError: () => {
    toast.error('An error occurred while creating the adoption status')
  },
})
...
<template>
  <div class="flex flex-row items-center justify-between">
    <DefaultTitle text="Adoption Statuses" />
    <Dialog v-model:open="createFormOpen">
      ...
      <DialogContent class="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle> Create a adoption status </DialogTitle>
          <DialogDescription> Add a new adoption status to the system. </DialogDescription>
        </DialogHeader>
        <form id="createAdoptionStatusForm" @submit.prevent="onSubmit">
        ...
        <DialogFooter>
          <Button type="submit" form="createAdoptionStatusForm">Create</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
